...
public class TestServer {
    ...
    private static void loadQuestionsFromFile(String filename) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                } else if (currentCorrectAnswers.size() > 1 && !currentCorrectAnswers.contains("None of the above/below")) {
                    // If there are multiple correct answers, add them to the answers list
    ...
    private static void sendQuestionPage(PrintWriter out, String question, List<String> questionAnswers,
        ...
        //out.println("  var blockQuote = document.getElementById('blockQuote');");
        ...
        out.println("    } else {");
        out.println("      feedback.innerHTML = 'Incorrect answer. The correct answer(s) is/are: " + String.join(", ", correctAnswers) + "';");
        ...
        out.println("  } else {");
        // out.println("    source.innerHTML = 'Some network guy probably ';");
        // out.println("    blockQuote.style.display = 'block';");
        ...
        for (int i = 0; i < questionAnswers.size(); i++) {
            ...
            out.println("<input class='form-check-input' type=\"radio\" name=\"answer\" value=\"" + questionAnswers.get(i) + "\">" + questionAnswers.get(i) + "<br/>");
            out.println("</div>");
        }
        ...
        out.println("<button id=\"nextQuestionButton\" style=\"display:none;\" onclick=\"location.reload();\" class='button-group-button btn btn-warning m-1'>Answer Another Question</button>");
        out.println("<button id=\"quitButton\" onclick=\"window.location.href='/goodbye';\" class='button-group-button btn btn-danger m-1'>Quit</button>");
        out.println("</div>");
        out.println("</div>");

        // DO NOT CLICK BUTTON
        out.println("<button onclick=\"window.location.href='/do-not-click';\" class='btn btn-outline-danger m-1 position-bottom-right'>DO NOT CLICK</button>");
        out.println("</body>");
        out.println("</html>");
    }
